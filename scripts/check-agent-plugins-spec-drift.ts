...
const DEFAULT_PLUGIN_JSON = join(REPO_ROOT, "plugins/team-harness/plugin.json");
...
Options:
  --plugin-json <path>  Path to plugin.json (default: plugins/team-harness/plugin.json)
...
export function parseAgentPluginsSpecDriftArgs(args: string[]): ParsedDriftCheckArgs {
  const options: ParsedDriftCheckArgs = {
    pluginJsonPath: DEFAULT_PLUGIN_JSON,
    jsonOutput: false,
  };
