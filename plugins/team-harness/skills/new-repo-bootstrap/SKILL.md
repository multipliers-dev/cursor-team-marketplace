...
| --- | --- | --- |
| **User Rules** | Short always-on invariants from `docs/engineering-invariants.md` in the installed **team-harness** plugin (authority, topology, stop-after-open, do-not-defer, tools/comms) | Paste via Customize → Rules → User once per account; not org-enforced; not read from the repo being bootstrapped |
| **Planning methodology** | `/planning-methodology` — full staged-plan procedure | Available when the **team-harness** plugin is installed |
| **Inherited MCP connectors** | MCP servers already on this Cursor account (when configured) | Do not duplicate in-repo MCP configs for connectors you already have — authenticate them instead; prefer `gh` for GitHub writes; do not invent a second GitHub integration; skip MCP steps when none are configured |
...
1. **GitHub App** *(optional)* — if your org uses a Cursor GitHub App and it is not scoped to “All repositories”, grant the new repo access. If no App is configured on this account, skip and say so — do not require another repo or org setup doc.
...
4. **Domain rules / planning supplements** — only if this domain is special (API contracts, facts-vs-prose, route checklists). Do not promote product checklists into the User Rules paste (`docs/engineering-invariants.md` from the installed plugin).
...
7. **Bugbot** *(optional)* — if Bugbot is available on this account, keep **manual invoke only**. Do not enable automatic runs on every PR. Add `.cursor/BUGBOT.md` only if this repo needs an on-demand review guide. If Bugbot is not configured, skip and say so.
...
| Layer | Role |
| --- | --- |
| User Rules | Always-on agent invariants (`docs/engineering-invariants.md` paste from the installed plugin) |
...
- Do not invent skills that are not already in this plugin
- Do not clone or read other repos (including cursor-team-marketplace) to obtain harness files
...
When asked to bootstrap a new repo: walk this checklist, add only the in-repo items that apply, skip optional integrations (GitHub App, Bugbot, MCP) when this account does not have them and say so, offer the thin GitHub PR template when the repo will use GitHub PRs and lacks one, invoke `/cloud-hooks-bootstrap` for Husky repos (including `environment.json` when Cloud Agents are expected), leave intentional no-harness / planning-only proof repos unharnessed, open **Open PR only** changes targeting `main`, and state clearly what was inherited vs added in-repo.
